...
class TC_HSTAT_2_1(HSTATBase):
    ...
    async def test_TC_HSTAT_2_1(self):
        """[TC-HSTAT-2.1] Primary functionality with DUT as Server"""
        ...
        self.step(3, "TH reads from the DUT the SupportedModesMode attribute.",
                  expectation="Verify that the DUT response contains a list with up to 4 entries. Verify: "
        ...
        self.step(4, "TH reads from the DUT the Mode attribute.",
                  expectation="Verify that the DUT response contains a value between 0 and 3 inclusive.")
        ...
        self.step(5, "TH reads from the DUT the SystemState attribute.",
                  expectation="Verify that the DUT response contains a value between 0 and 3 inclusive.")
        ...
        if self.sensorFeatureSupported:
            self.step(6, "TH reads from the DUT the MinSetpoint attribute.",
                      expectation="Verify that the DUT response contains a value between 0 and 99 inclusive. Store the value as MinSetpointValue")
            ...
            self.step(7, "TH reads from the DUT the MaxSetpoint attribute.",
                      expectation="Verify that the DUT response contains a value between MinSetpointValue + 1 and 100 inclusive. Store the value as MaxSetpointValue.")
            ...
            self.step(8, "TH reads from the DUT the Step attribute.",
                      expectation="Verify that the DUT response contains a value between 1 and 100 inclusive such that (MaxSetpointValue - MinSetpointValue) % value == 0. "
            ...
            self.step(9, "TH reads from the DUT the UserSetpoint attribute.",
                      expectation="Verify that the DUT response contains a value between MinSetpointValue and MaxSetpointValue inclusive such that (SetpointValue - MinSetpointValue) % StepValue == 0. "
        ...
        if self.sensorFeatureSupported and (self.attributes.TargetSetpoint.attribute_id in self.supported_attributes):
            self.step(10, "If the TargetSetpoint attribute is supported then TH reads from the DUT the TargetSetpoint attribute, otherwise skip this step.",
                      expectation="Verify that the DUT response contains a value between MinSetpointValue and MaxSetpointValue inclusive.")
        ...
        if self.humidifierFeatureSupported:
            self.step(11, "TH reads from the DUT the MistType attribute.",
                      expectation="Verify that the DUT response contains a value with at most the 2 least significant bits set.")
        ...
        if self.continuousFeatureSupported:
            self.step(12, "TH reads from the DUT the Continuous attribute.",
                      expectation="Verify that the DUT response contains a Boolean.")
        ...
        if self.attributes.Sleep.attribute_id in self.supported_attributes:
            self.step(13, "TH reads from the DUT the Sleep attribute.",
                      expectation="Verify that the DUT response contains a Boolean.")
        ...
        if self.optimalFeatureSupported:
            self.step(14, "TH reads from the DUT the Optimal attribute.",
                      expectation="Verify that the DUT response contains a Boolean.")
        ...
        if self.condPumpFeatureSupported:
            self.step(15, "TH reads from the DUT the CondPumpEnabled attribute.",
                      expectation="Verify that the DUT response contains a Boolean.")
        ...
        if self.condPumpFeatureSupported:
            self.step(16, "TH reads from the DUT the CondRunCount attribute.",
                      expectation="Verify that the DUT response contains a uint16.")
